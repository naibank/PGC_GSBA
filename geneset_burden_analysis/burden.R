                           ...
                           BiasedUrn = F) 
{
  ...
  for (cnvtype in distinct.prefixes) {
    for (this.gs in names(geneset)) {
      ...
      # ref.model <- clogit(status ~ sex + PC1 + PC2 + PC3 + 
      #                       strata(PLATFORM) + global_1 + global_2, cnv.matrix)
      # add.model <- clogit(status ~ sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + 
      #                       strata(PLATFORM) + global_1 + global_2 + feature, cnv.matrix)
      
      ref.term <- sprintf("%s ~ %s", label, paste(c(this.covariates, "global"), 
                                                  collapse = " + "))
      add.term <- sprintf("%s + feature", ref.term)
      
      ref.model <- glm(ref.term, cnv.matrix, family = binomial)
      add.model <- glm(add.term, cnv.matrix, family = binomial)
