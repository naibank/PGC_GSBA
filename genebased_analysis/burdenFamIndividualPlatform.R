...
CNVLociTest <- function (cnv.table, cnv.matrix, annotation.table, label, covariates, 
  ...
  for (cnvtype in cnvtypes) {
    ...
    dt.out <- data.frame()
    dt.temp <- cnv.matrix[, c("sample", "family.ID", label, covariates)]
    ...
    ref.model <- clogit(status ~ sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + strata(family.ID), dt.temp)
    ...
    for (iloci in 1:nrow(new.loci)) {
      ...
      add.model <- clogit(status ~ sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + strata(family.ID) + gene_count, dt.temp)
    ...
    write.table(dt.out, sprintf("intermidiate.%s.xd.dt.out.%s.tsv", cnvtype, Sys.Date()), sep="\t",row.names=F, quote=F, col.names=T)
...
names(sampleinfo) <- c("family.ID", "sample", "status", "sex", "ancestry", "PLATFORM", "dataset", "phenotype", paste0("PC", 1:10))
...
for(platform in unique(cnv.matrix$PLATFORM)){
  ...
  loci.test.out <- CNVLociTest(cnvs.in, cnv.matrix[which(cnv.matrix$PLATFORM == platform), ], genes, "status", covariates, permutation = F, nsubject = 1, cnvtypes = params$cnvtypes)
